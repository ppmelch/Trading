...
def hyperparams(trial) -> dict:
    """
    Defines the hyperparameter search space for the backtesting strategy.
    ...
    -------
    dict
        Dictionary containing hyperparameters and their suggested values.

    Hyperparameters
    ---------------
    rsi_window : int
        Lookback period for RSI calculation (5-50).
    rsi_lower : int
        RSI threshold for generating buy signals (5-35).
    rsi_upper : int
        RSI threshold for generating sell signals (65-95).
    momentum_window : int
        Lookback period for momentum calculation (5-50).
    momentum_threshold : float
        Threshold for generating momentum buy/sell signals (0.1-2.0).
    volatility_window : int
        Lookback period for volatility calculation (5-50).
    volatility_threshold : float
        Threshold for generating volatility buy/sell signals (0.01-0.5).
    stop_loss : float
        Stop-loss ratio for positions (0.01-0.2).
    take_profit : float
        Take-profit ratio for positions (0.2-0.3).
    n_shares : int
        Number of shares to trade per position (1-50).
    """
    return {
